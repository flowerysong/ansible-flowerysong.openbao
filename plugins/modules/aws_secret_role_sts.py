...
short_description: Manage AWS STS secret roles in OpenBao
description:
  - Manage AWS STS secret roles in OpenBao.
  - The underlying API is the same one used by C(aws_secret_role_iam), so either
    module can delete roles created by the other.
seealso:
  - module: flowerysong.openbao.aws_secret_role_iam
version_added: 0.2.0
...
options:
  ...
  role_arns:
    ...
    elements: str
    description:
      - AWS roles this OpenBao role is allowed to assume.
