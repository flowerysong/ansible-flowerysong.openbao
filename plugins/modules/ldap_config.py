...
short_description: Manage LDAP authentication in OpenBao
description:
  - Manage LDAP authentication in OpenBao.
version_added: 0.2.0
...
- name: Configure LDAP authentication
  flowerysong.openbao.ldap_config:
...
def main():
    ...
    optspec.update(
        dict(
            ...
            groupfilter=dict(default='(|(memberUid={{.Username}})(member={{.UserDN}})(uniqueMember={{.UserDN}}))'),
            groupdn=dict(),
            groupattr=dict(
                default='cn',
            ),
    ...
    changed = False
    diff = openbao_compare(config, new_config, ignore_keys=['request_timeout'])
    if diff:
        changed = True
