...
short_description: Manage KV v2 secrets in OpenBao
description:
  - Manage KV v2 secrets in OpenBao.
version_added: 0.1.0
...
def main():
    ...
    path = f'{module.params["mount_point"].rstrip("/")}/data/{module.params["key"]}'
    ...
    if module.params['state'] == 'present' and result.get('data', {}).get('data') != module.params['value']:
        ...
        changed = True
        if not module.check_mode:
            client.delete(f'{module.params["mount_point"].rstrip("/")}/metadata/{module.params["key"]}')
