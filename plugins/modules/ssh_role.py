...
short_description: Manage SSH roles in OpenBao
description:
  - Manage SSH signing roles in OpenBao.
...
options:
  ...
  key_id_format:
    description:
      ...
      - OpenBao allows the variables C({{token_display_name}}), C({{role_name}}), and C({{public_key_hash}}) to be used.
    type: str
    default: ''
  ...
  algorithm_signer:
    description:
      ...
      - Note that the default of C(rsa-sha2-512) is different from the upstream default.
    type: str
    choices:
      - ssh-rsa
      - rsa-sha2-256
  ...
  issuer_ref:
    description:
      - Specifies the issuer reference to use for signing requests.
    type: str
    default: default
"""
...
class SSHRoleModule(OpenBaoModule):
