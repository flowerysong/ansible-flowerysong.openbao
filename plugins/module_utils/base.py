...
def openbao_token_argument_spec():
    return dict(
        token_ttl=dict(
            type='int',
            default=0,
...
def openbao_compare(dict1, dict2, ignore_keys=None, unsorted_keys=None):
    diff = {}
...
class OpenBaoClient:
    def __init__(self, params, module=None):
        self._module = module
        self.params = params
        self.params['openbao_addr'] = params['openbao_addr'].rstrip('/')
        if not params['token']:
    ...
    def _open_url(self, path, method, data=None, fatal=True):
        url = f'{self.params["openbao_addr"]}/v1/{path}'
